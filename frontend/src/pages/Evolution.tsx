    ...
    <div className="space-y-6">
      <div className="page-header flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="page-sub">Evolve the 8-gene chromosome via genetic algorithm</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="card text-center">
          ...
          <p className={`font-semibold capitalize ${statusColor}`}>{status}</p>
        </div>
        <div className="card text-center">
          <p className="label mb-2">Generation</p>
          <p className="font-mono text-xl text-zinc-100">{history.length}</p>
        </div>
        <div className="card text-center">
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-zinc-500 font-mono">Diversity: {(diversity * 100).toFixed(1)}%</span>
        </div>
        {history.length > 0
          ? <FitnessChart data={history} />
          : <div className="h-48 flex items-center justify-center text-zinc-600 text-sm">Start evolution to see live chart</div>
        }
      </div>

      {displayGenes && (
        <div className="card">
          <h2 className="section-title mb-4">Chromosome Genes</h2>
