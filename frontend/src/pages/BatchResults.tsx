  ...
  return (
    <div className="space-y-6">
      <div className="page-header">
        ...
        <p className="page-sub">Run decisions for all 200 users simultaneously</p>
      </div>
      ...
      {rows.length > 0 && (
        <>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="card">
              <h2 className="section-title mb-3">Decision Distribution</h2>
            ...
            <div className="card">
              <h2 className="section-title mb-3">Counts</h2>
              <div className="space-y-2.5">
                {Object.entries(counts).map(([d, n]) => (
                  <div key={d} className="flex items-center gap-3">
                    ...
                    <span className="font-mono text-sm w-8 text-right text-zinc-400">{n}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="card">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left border-b border-violet-900/30">
                    <th className="pb-2 text-zinc-500 font-semibold">User</th>
                    <th className="pb-2 text-zinc-500 font-semibold">Age</th>
                    <th className="pb-2 text-zinc-500 font-semibold">Decision</th>
                    <th className="pb-2 text-zinc-500 font-semibold">Score</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.slice(0, 50).map((r) => (
                    <tr key={r.user_id} className="border-b border-violet-900/20 hover:bg-violet-900/10 transition-colors">
                      <td className="py-2 text-zinc-300">{r.user_name}</td>
                      <td className="py-2 text-zinc-500">{r.age_group}</td>
                      <td className="py-2"><DecisionBadge decision={r.decision} size="sm" /></td>
                      <td className="py-2 font-mono text-zinc-500">{r.combined_score.toFixed(3)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {filtered.length > 50 && <p className="text-xs text-zinc-500 mt-2">Showing 50 of {filtered.length}</p>}
            </div>
          </div>
        </>
      )}
